...
local function moveEnvelopePointsRandomly(envelope, time_start, time_end, time_min, time_max, value_min, value_max)
    ...
    for i = 0, num_points - 1 do
        ...
        if selected and time >= time_start and time <= time_end then
            ...
            local add_value = value_min + math.random() * (value_max - value_min)
...
local function main()
    local ok, retvals_csv = reaper.GetUserInputs("Randomly move envelope points (add values)", 4, "Time minimum (ms),Time maximum (ms),Value minimum,Value maximum", "-100,100,-0.2,0.2")
    ...
    reaper.Undo_EndBlock("Randomly move envelope points in selected area (add values)", -1)
